...
| USA-NPN phenology | **v1** (Wave 3) | CC BY 4.0 (quoted 2026-09-11) | citation string per data type; post-publication notice requested | "Data were provided by the USA National Phenology Network and the many participants who contribute to its Nature's Notebook program." | usanpn.org/about/terms |
| iNaturalist via GBIF | **v1 subset** (Wave 3) | dataset-level CC BY-NC, **per-record CC0 / CC BY** (2026: 1.02 M CC0, 2.38 M CC BY) | existing per-record filter; media excluded; never de-obscure | per record via GBIF dataset 50c9509d + observer | api.gbif.org dataset record |
...
| Xeno-canto | Wave 4 (credentialed) | API v3 returns 401 without key (probed 2026-09-11); per-recording CC, SA/NC/ND excluded by policy | key server-side; hotlink audio; attribute recordist | per recording | xeno-canto.org/api/3 |
...
| CDC NWSS (data.cdc.gov 2ew6-ywp6) | v1-cond (site series, county-level) | no licence field in Socrata metadata (US-gov CDC data); rows carry county FIPS + wwtp_id, **no coordinates** | render as county polygons/centroids only; verify data.cdc.gov terms | "CDC National Wastewater Surveillance System" | data.cdc.gov/api/views/2ew6-ywp6 |
...
| USDA APHIS HPAI wild birds | v1-cond (polygons) | US-gov page; **no official download/API found** (ArcGIS search returns third-party mirrors only) | ask APHIS for the feature service; do not scrape mirrors | "USDA APHIS" | aphis.usda.gov HPAI wild birds page |
